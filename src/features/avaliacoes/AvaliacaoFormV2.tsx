...
export function AvaliacaoFormV2({ alunoId, alunoNome, alunoAtivo, referencia, dataPadrao }: Props) {
  ...
  const chaveFormulario = estado.status === "erro" ? `erro:${estado.tentativa}` : estado.status;
  ...
  return <main className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6"><Link href={`/alunos/${alunoId}`} className="text-sm underline-offset-4 hover:underline">← Voltar</Link><div className="mt-4 flex items-center gap-2"><h1 className="text-2xl font-semibold">Nova avaliação</h1>{!alunoAtivo ? <Badge variant="outline">Inativo</Badge> : null}</div><p className="mt-1 text-muted-foreground">{alunoNome}</p>
    <form key={chaveFormulario} ref={formRef} action={dispatch} className="mt-6 flex flex-col gap-6" onInput={() => { if (timerRef.current) clearTimeout(timerRef.current); timerRef.current = setTimeout(() => { if (formRef.current) gravarRascunho(alunoId, formDataParaValoresV2(new FormData(formRef.current))); }, 800); }} onSubmit={(e) => { if (emAndamentoRef.current) e.preventDefault(); else emAndamentoRef.current = true; }}>
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row"><Button type="submit" disabled={pendente} className="h-11 sm:h-9">{pendente ? "Salvando…" : "Salvar avaliação"}</Button><Link href={`/alunos/${alunoId}`} className={cn(buttonVariants({ variant: "outline" }), "h-11 sm:h-9")}>Cancelar</Link></div>
    </form></main>;
}
